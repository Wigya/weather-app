...
function SearchBar({inputCity, setInputCity, apiData, setApiData, setLatitudeLongitude} : props) {
    ...
    return (
        <div className="flex items-center w-full flex-col">
            <div className="flex mt-5">
                <input type="text" placeholder="Enter city name..." onChange={handleChange} autoComplete="off" className="w-64 h-12 p-2 focus:outline-none" value={inputCity} name='inputCity' />
                {inputCity.length === 0 ? <Search32 className="text-black h-12 pr-1 bg-white cursor-pointer"/> : <MisuseOutline32 className="text-black h-12 pr-1 bg-white cursor-pointer" onClick={clearInput} />}
            </div>
            <div className="mt-1">
